...
<main class="flex-1 overflow-y-auto p-6">
    ...
    <div class="max-w-3xl mx-auto">
        <form action="{{ route('admin.category.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-xl shadow-sm border border-gray-100 p-8 space-y-6">
        @csrf
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Category Name *</label>
                    <input type="text" id="name" name="name" placeholder="e.g. Electronics" class="w-full border px-4 py-2 rounded-lg outline-none focus:ring-1 focus:ring-primary" value="{{ old('name') }}">
                    @error('name')
                ...
                <div>
                    ...
                    <input type="text" id="slug" name="slug" placeholder="electronics" class="w-full border px-4 py-2 rounded-lg bg-gray-50 outline-none" value="{{ old('slug') }}" readonly>
                    @error('slug')
                        <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div>
                ...
                <div class="relative flex items-center justify-center w-full h-40">
                    ...
                    <label for="category-image" class="relative flex flex-col items-center justify-center w-full h-full border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100 transition overflow-hidden">
                        ...
                        <img id="image-preview" class="hidden absolute inset-0 w-full h-full object-contain p-2 z-20 bg-white" src="" alt="Image Preview">
            ...
            <div class="flex items-center gap-2">
                <input type="checkbox" id="status" name="status" class="w-4 h-4 text-primary rounded border-gray-300 focus:ring-primary" value="1" {{ old('status') ? 'checked' : '' }}>
            ...
            <div class="flex justify-end gap-3 pt-4 border-t">
                ...
                <button type="submit" class="px-6 py-2 bg-primary text-white rounded-lg hover:bg-blue-600 transition text-sm font-medium shadow-sm">Save Category</button>
            </div>
        </form>
    </div>
</main>
