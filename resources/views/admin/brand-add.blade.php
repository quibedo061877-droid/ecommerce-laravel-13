...
<main class="flex-1 overflow-y-auto p-6">
    ...
    <div class="max-w-3xl mx-auto">
        <form action="{{ route('admin.brand.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-xl shadow-sm border border-gray-100 p-8 space-y-6">
        @csrf
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Brand Name *</label>
                    <input type="text" id="name" name="name" placeholder="e.g. Samsung" class="w-full border px-4 py-2 rounded-lg outline-none focus:ring-1 focus:ring-primary" value="{{ old('name') }}">
                    @error('name')
                ...
                <div>
                    ...
                    <input type="text" id="slug" name="slug" placeholder="samsung" class="w-full border px-4 py-2 rounded-lg bg-gray-50 outline-none" value="{{ old('slug') }}" readonly>
                    @error('slug')
                        <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                    @enderror
                </div>
